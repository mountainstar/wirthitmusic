import { Box, Grid } from "@mui/material";
import { FC } from "react";
interface IProps {};

const Footer: FC<IProps> = (props) => {
    return (
        <Box
            component="footer"
            sx={{
                height: '220px',
                backgroundColor: '#222',
                color: '#fff',
                px: { xs: 2, sm: 4 },
                py: 4,
                display: 'flex',
                alignItems: 'center',
            }}
        >
            <Grid
                container
                spacing={2}
                alignItems="center"
                justifyContent="space-between"
                sx={{
                    width: '100%',
                    maxWidth: '900px',
                    margin: '0 auto',
                }}
            >
                {/* Left: Page Section Links */}
                <Grid size={{ xs: 12, sm: 5 }} >
                    <Box
                        sx={{
                            display: 'flex',
                            flexDirection: 'column',
                            gap: 1.2,
                        }}
                    >
                        <a href="#about" style={{ color: '#fff', textDecoration: 'none', fontSize: '1.1rem' }}>About</a>
                        <a href="#contact" style={{ color: '#fff', textDecoration: 'none', fontSize: '1.1rem' }}>Contact</a>
                        <a href="#services" style={{ color: '#fff', textDecoration: 'none', fontSize: '1.1rem' }}>Services</a>
                    </Box>
                </Grid>
                {/* Right: Social Media Links */}
                <Grid size={{ xs: 12, sm: 5 }} >
                    <Box
                        sx={{
                            display: 'flex',
                            flexDirection: 'row',
                            justifyContent: { xs: 'flex-start', sm: 'flex-end' },
                            gap: 2,
                        }}
                    >
                        <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" style={{ color: '#fff' }}>
                            <svg width="28" height="28" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M22.675 0h-21.35C.595 0 0 .592 0 1.326v21.348C0 23.406.595 24 1.325 24h11.495v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.797.143v3.24l-1.918.001c-1.504 0-1.797.715-1.797 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116C23.406 24 24 23.406 24 22.674V1.326C24 .592 23.406 0 22.675 0"/>
                            </svg>
                        </a>
                        <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" style={{ color: '#fff' }}>
                            <svg width="28" height="28" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M24 4.557a9.93 9.93 0 0 1-2.828.775 4.932 4.932 0 0 0 2.165-2.724c-.951.564-2.005.974-3.127 1.195a4.92 4.92 0 0 0-8.384 4.482C7.691 8.095 4.066 6.13 1.64 3.161c-.542.929-.856 2.01-.857 3.17 0 2.188 1.115 4.116 2.823 5.247a4.904 4.904 0 0 1-2.229-.616c-.054 2.281 1.581 4.415 3.949 4.89a4.936 4.936 0 0 1-2.224.084c.627 1.956 2.444 3.377 4.6 3.417A9.867 9.867 0 0 1 0 21.543a13.94 13.94 0 0 0 7.548 2.209c9.057 0 14.009-7.496 14.009-13.986 0-.21 0-.423-.016-.634A9.936 9.936 0 0 0 24 4.557z"/>
                            </svg>
                        </a>
                        <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" style={{ color: '#fff' }}>
                            <svg width="28" height="28" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 1.366.062 2.633.334 3.608 1.308.974.974 1.246 2.241 1.308 3.608.058 1.266.07 1.646.07 4.85s-.012 3.584-.07 4.85c-.062 1.366-.334 2.633-1.308 3.608-.974.974-2.241 1.246-3.608 1.308-1.266.058-1.646.07-4.85.07s-3.584-.012-4.85-.07c-1.366-.062-2.633-.334-3.608-1.308-.974-.974-1.246-2.241-1.308-3.608C2.175 15.647 2.163 15.267 2.163 12s.012-3.584.07-4.85c.062-1.366.334-2.633 1.308-3.608.974-.974 2.241-1.246 3.608-1.308C8.416 2.175 8.796 2.163 12 2.163zm0-2.163C8.741 0 8.332.013 7.052.072 5.775.13 4.602.402 3.635 1.37c-.967.967-1.24 2.14-1.298 3.417C2.013 5.668 2 6.077 2 9.333v5.334c0 3.256.013 3.665.072 4.945.058 1.277.331 2.45 1.298 3.417.967.967 2.14 1.24 3.417 1.298C8.332 23.987 8.741 24 12 24s3.668-.013 4.948-.072c1.277-.058 2.45-.331 3.417-1.298.967-.967 1.24-2.14 1.298-3.417.059-1.28.072-1.689.072-4.945V9.333c0-3.256-.013-3.665-.072-4.945-.058-1.277-.331-2.45-1.298-3.417-.967-.967-2.14-1.24-3.417-1.298C15.668.013 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zm0 10.162a3.999 3.999 0 1 1 0-7.998 3.999 3.999 0 0 1 0 7.998zm6.406-11.845a1.44 1.44 0 1 0 0 2.88 1.44 1.44 0 0 0 0-2.88z"/>
                            </svg>
                        </a>
                        <a href="https://soundcloud.com" target="_blank" rel="noopener noreferrer" style={{ color: '#fff' }}>
                            <svg width="28" height="28" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M17.5 17.5c-1.1 0-2-.9-2-2 0-.1 0-.2.1-.3-.2-.1-.4-.2-.6-.2-.2 0-.4.1-.6.2.1.1.1.2.1.3 0 1.1-.9 2-2 2s-2-.9-2-2c0-.1 0-.2.1-.3-.2-.1-.4-.2-.6-.2-.2 0-.4.1-.6.2.1.1.1.2.1.3 0 1.1-.9 2-2 2s-2-.9-2-2c0-.1 0-.2.1-.3-.2-.1-.4-.2-.6-.2-.2 0-.4.1-.6.2.1.1.1.2.1.3 0 1.1-.9 2-2 2s-2-.9-2-2c0-1 .7-1.8 1.7-2 .2-3.3 2.8-6 6.1-6 .7 0 1.4.1 2 .3.2 0 .3.2.3.4v7.3c0 .1.1.2.2.2.1 0 .2-.1.2-.2v-7.3c0-.2.1-.4.3-.4.6-.2 1.3-.3 2-.3 3.3 0 6 2.7 6.1 6 .9.2 1.7 1 1.7 2 0 1.1-.9 2-2 2z"/>
                            </svg>
                        </a>
                    </Box>
                </Grid>
            </Grid>
        </Box>
    );
}
export default Footer;